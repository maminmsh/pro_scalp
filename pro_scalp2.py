...
def fast_linreg(x):
    """
    محاسبات خط رگرسیون خطی ساده (y = slope * i + intercept)
    بدون استفاده از np.polyfit، بسیار سریع
    """
    n = len(x)
    X = np.arange(n)
    sum_x = X.sum()
...
# -------------------------------
# تابع اصلی پیش‌بینی AI
# -------------------------------
def calculate_ai_forecast(close: pd.Series, oscillator: pd.Series):
    """
    close: قیمت پایانی سری زمانی
    oscillator: اندیکاتور اسیلاتور (مثلاً RSI یا Stochastic)

    خروجی: ai_bias, ai_osc
    """
    # Rolling regression روی قیمت
    ai_bias = close.rolling(window=config.ai_length).apply(fast_linreg, raw=True)

    # Rolling regression روی اسیلاتور
    ai_osc = oscillator.rolling(window=config.ai_length).apply(fast_linreg, raw=True)

    # پر کردن مقادیر اولیه NaN
    ai_bias = ai_bias.fillna(0.0)
    ai_osc = ai_osc.fillna(0.0)

    return ai_bias, ai_osc
...
def calculate_htf_trend(close, htf_data):
    ...
    if config.useHTF:
        ...
        htf_hma = htf_hma.reindex(close.index, method="ffill")
...
# ─────────────────────────────────────────────
def pro_scalper_ai2(df, htf_data=None):
